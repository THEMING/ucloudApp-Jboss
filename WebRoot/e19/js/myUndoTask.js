...
//                        }),
                        ...
                        listeners:{
                            select: function(combo, record, index){
                                myUndoTask.taskTypeOnSelect(combo);
...
//                                         fieldLabel : '工单状态',
                                         ...
                                         value:myUndoTaskUrgencyLevelQry==""?"":null,
                                         anchor : '100%',
                                         listeners:{
                                            select: function(combo, record, index){   
                                                
                                                Ext.getCmp('urgencyLevelQry').setValue(combo.value);
                                                myUndoTaskUrgencyLevelQry = combo.value;
                                            }
                                        }
                                    },{
                                        xtype:'hidden',
...
//            }]
        ...
        taskTypeStore.load();
//        this.showObject(taskTypeStore.data);
//        var firstValue = Ext.getCmp('taskType').store.getRange()[0].data.value;//这种方法可以获得第一项的值 
//        alert(firstValue);
...
//        Ext.getCmp('processDefId').setValue(combo.value.substring(0,combo.value.indexOf("$$$$")));
    ...
    this.qryListGrid = function(param) {
        ...
        Ext.getCmp('undoTaskGrid').store.on('load', function(store,records) {
//        	Ext.getCmp('undoTaskGrid').getSelectionModel().selectFirstRow();// 选中第一行
            store.each(function(record,rowIndex) {
            	if(record.data.taskWarning && record.data.taskWarning == 1){
            	   Ext.getCmp('undoTaskGrid').getView().getRow(rowIndex).style.backgroundColor = '#FFAAAA';// 超时红色
            	                    }
            	                });
...
//                  }
                  ,width:gridWidth*0.15},
                 {header:'URL',dataIndex:'formUrl',hidden:true},
                 {header:'currentState',dataIndex:'currentState',width:gridWidth*0.15,hidden:true},
                 {header:'任务类型',dataIndex:'processModelId',width:gridWidth*0.1,hidden:true},
                 {header:'任务类型',dataIndex:'flowType',width:gridWidth*0.1},
                 {header:'任务类型',dataIndex:'processModelName',hidden:true},
                 {header:'流程实例ID',dataIndex:'processInstID',width:gridWidth*0.1,hidden:true},
                 {header:'环节名称',dataIndex:'activityInstName',width:gridWidth*0.1},
                 {header:'环节ID',dataIndex:'activityInstID',width:gridWidth*0.1,hidden:true},
                 {header:'工单ID',dataIndex:'workOrderId',width:gridWidth*0.1,hidden:true},
                 {header:'派发人',dataIndex:'senderID',width:gridWidth*0.1,hidden:true},
                 {header:'任务实例创建时间',dataIndex:'createDate',width:gridWidth*0.15},
                 {header:'任务期望完成时间',dataIndex:'endDate',width:gridWidth*0.15},
                 {header:'是否超时',dataIndex:'taskWarningName',width:gridWidth*0.1,renderer:function(value,cellmeta,record,rowIndex,columnIndex,store){
                	 if(record.data.taskWarning==1){
...
//            collapsible : true,//是否可以收缩
            url: PATH+'/e19/tcmBpsAction.json?method=getMyUndoTasks',
            sm :cm                       
        });
        return grid;
    ...
    this.doDetail = function (rowIndex){
        ...
        var formUrll = "";
        formUrll = param.formUrl.substring(33);//因为流程环节的url地址为/e19/myUndoTaskUnify.jsp?formUrl=开头，共33个字符，需要截取位置33后所有信息作为环节标记
        processModelId = param.processModelName;
        if(formUrll=='0'){//清查以外修改环节
//            oper.detail("mod");
            testCardOrderModify.showWinPre(param.processInstID,param.activityInstID,param.taskInstID,formUrll);
        }else if(formUrll=='checkModify'){//清查修改环节
        	testCardOrderModify.showWinPre(param.processInstID,param.activityInstID,param.taskInstID,formUrll);
//            manager.showWinPre(param.processInstID,param.activityInstID,param.taskInstID,param.formUrl);
        }else{
            manager.showWinPre(param.processInstID,param.activityInstID,param.taskInstID,formUrll,param.workOrderId);
        }
//        /e19/myUndoTaskUnify.jsp?formUrl=
