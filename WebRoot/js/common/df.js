...
function writeValidateInfoAfterObject_df(info, thisObj) {  //写校验信息
	...
	if(!jQuery("#"+thisObj.id+"_error")[0]){
		...
		+'	<div style="" class="inputErrorInfoDiv"></div><div class="inputErrorInfoSpan">'+info+'</div>'  
		+'</div>';*/
		html = '<div id="'+thisObj.id+'_error" style="margin-top:'+height+';*+margin-top:'+height_ie7+';margin-left:'+left+';*+margin-left:'+left_ie7+';z-index:'+zIndex+';position:absolute;" class="inputErrorInfo">'
		+'	<table cellspacing="0" cellpadding="0" ><tr><td ><div class="inputErrorInfoDiv"></div></td><td><div class="inputErrorInfoSpan">'+info+'</div></td></tr></table>'  
		+'</div>';
...
function hideValidateInfo_df(thisInput){
	jQuery("#"+thisInput.id+"_error").remove();
