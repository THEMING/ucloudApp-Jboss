...
<%@page import="com.unicom.ucloud.df.tree.init.RmConfig"%>
<%@page import="com.unicom.ucloud.df.tree.tool.RmStringHelper"%><%
	...
	if(isOnlyLeaf == null || isOnlyLeaf == "" || isOnlyLeaf == "0"  ) {
		isOnlyLeaf = "0";
	}
	
	String queryCondition = request.getParameter("queryCondition");
	if(queryCondition == null || queryCondition == "" || queryCondition == "0"  ) {
		queryCondition = "";
	}
...
<script type="text/javascript" src="<%=request.getContextPath()%>/common/deeptree/js/deeptree.js"></script>
<script type="text/javascript">
	...
	var isOnlyLeaf = <%=isOnlyLeaf%>;  //返回值是否只含有叶子节点
	var queryCondition = "<%=queryCondition%>";  //异步查询条件
	...
	//定义全局路径
	var treeImagePath = "<%=request.getContextPath()%>/common/deeptree/image";  //树图标路径


	var xslPath = "<%=request.getContextPath()%>/common/deeptree/deeptree_xsl.jsp?inputType=" + inputType + "&treeImagePath=" + treeImagePath ;  //xslt文件相对路径
...
/**********************工具性方法 **********************/
	...
	function getMouseIdByEvent(eventId) {  //从事件对象获取某个对象的mapId
		var mapId = eventId;
		if(eventId.length > prefixSpan.length && eventId.substring(0, prefixSpan.length) == prefixSpan) {
			mapId = eventId.substring(prefixSpan.length);
		}
		return mapId;
	}
...
function filterHidden(checkedArray) {  //把节点中display:none的节点过滤掉
    var submitStringArray = new Array(0);
    for(var i=0; i<checkedArray.length; i++) {
        if(inputType == "radio"){   //增加radio情况的判断
            var thisCheckbox = getObjectById(prefixRadio + checkedArray[i]);
        }else{
...
function treeExpandNode(objNode){  //展开节点
    icon={
    ...
    if(parentNode != null){
        var oTreeImg=getObjectById(prefixTreeImg + parentNode.id.substring(prefixDiv.length));
        var oSpan=getObjectById(prefixSpan + parentNode.id.substring(prefixDiv.length));
        var oCheckbox=getObjectById(prefixCheckbox + parentNode.id.substring(prefixDiv.length));
        if(oTreeImg==null || oSpan==null)
            return false;
        var oChild=parentNode.nextSibling;
        ...
        if(oCheckbox != null) {
            parentNode.setAttribute("isSelected", oCheckbox.checked);
        }
    }
}
function treeNodeSelcet(objNode){
    var oCheckbox=getObjectById(prefixCheckbox + objNode.id.substring(prefixDiv.length));
    var oDivcheckbox=getObjectById(prefixCheckbox + objNode.id.substring(prefixDiv.length)+"div");
    if(oCheckbox != null){
        //jQuery(".TreeNode").removeClass("selected");查找到的节点变色
        //objNode.className="TreeNode selected";
        oDivcheckbox.className = "tree-firerift-style-o on";
        oCheckbox.checked = true;
    }
    var oRadio=getObjectById(prefixRadio + objNode.id.substring(prefixDiv.length));
    var oDivRadio=getObjectById(prefixRadio + objNode.id.substring(prefixDiv.length)+"div");
    if(oRadio != null){
        $('.tree-firerift-style-r').removeClass('on').addClass('off');
        oDivRadio.className = "tree-firerift-style-r on";
        oRadio.checked = true;
    } 
}
//获取父节点的name
function getParentName(childObj){ 
    var parentString = prefixDiv + gradeMapParent[childObj.id.substring(prefixDiv.length)];
    var returnObj = new Object();
    var tempObj = getObjectById(parentString);
    returnObj["parentName"] = tempObj.getAttribute("text");
    return returnObj;
}
