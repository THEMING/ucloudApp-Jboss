...
//树的返回值反写
function toDoWritePartyWindowTree(inputArray, rtObj) {
	var userText = inputArray[0];
	var userId = inputArray[1];
	var userContractNumber = inputArray[2];
	var userEmail = inputArray[3];
	var userSex = inputArray[4];
	var userParentId = inputArray[5];
	var userThisType = inputArray[6];
	var userAccountId = inputArray[7];
	var userAccountName = inputArray[8];
	var userLeaf = inputArray[9];

	if(inputArray != null && rtObj != null && rtObj.length > 0){
		var allUserText = "";
		var allUserId = "";
		var allUserContractNumber = "";
		var allUserEmail = "";
		var allUserSex = "";
		var allUserParentId  = "";
		var allUserThisType = "";
		var allUserAccountId  = "";
		var allUserAccountName= "";
		var allUserLeaf = "";
		for(var i=0; i<rtObj.length-1; i++) {
			allUserText += rtObj[i]['text'] + ",";
			allUserId += rtObj[i]['id'] + ",";
			allUserContractNumber += rtObj[i]['contractNumber'] + ",";
			allUserEmail += rtObj[i]['email'] + ",";
			allUserSex += rtObj[i]['sex'] + ",";
			allUserParentId += rtObj[i]['parentId'] + ",";
			allUserThisType += rtObj[i]['thisType'] + ",";
			allUserAccountId += rtObj[i]['accountId'] + ",";
			allUserAccountName += rtObj[i]['accountName'] + ",";
			allUserLeaf += rtObj[i]['leaf'] + ",";
		}
		allUserText += rtObj[rtObj.length-1]['text'] ;
		allUserId += rtObj[rtObj.length-1]['id'] ;
		allUserContractNumber += rtObj[rtObj.length-1]['contractNumber'] ;
		allUserEmail += rtObj[rtObj.length-1]['email'] ;
		allUserSex += rtObj[rtObj.length-1]['sex'] ;
		allUserParentId += rtObj[rtObj.length-1]['parentId'] ;
		allUserThisType += rtObj[rtObj.length-1]['thisType'] ;
		allUserAccountId += rtObj[rtObj.length-1]['accountId'] ;
		allUserAccountName += rtObj[rtObj.length-1]['accountName'] ;
		allUserLeaf += rtObj[rtObj.length-1]['leaf'];
		if(userText != null && userText != ""){
			//eval(userText).value = allUserText;
			document.getElementById(userText).value=allUserText;
		}
		if(userId != null && userId != ""){
			//eval(userId).value = allUserId;
			document.getElementById(userId).value=allUserId;
		}
		if(userContractNumber != null && userContractNumber != ""){
			document.getElementById(userContractNumber).value=allUserContractNumber;
			//eval(userContractNumber).value = allUserContractNumber;
		}
		if(userEmail != null && userEmail != ""){
			document.getElementById(userEmail).value=allUserEmail;
			//eval(userEmail).value = allUserEmail;
		}
		if(userSex != null && userSex != ""){
			document.getElementById(userSex).value=allUserSex;
			//eval(userSex).value = allUserSex;
		}
		if(userParentId != null && userParentId != ""){
			document.getElementById(userParentId).value=allUserParentId;
			//eval(userParentId).value = allUserParentId;
		}
		if(userThisType != null && userThisType != ""){
			document.getElementById(userThisType).value=allUserThisType;
			//eval(userThisType).value = allUserThisType;
		}
		if(userAccountId != null && userAccountId != ""){
			document.getElementById(userAccountId).value=allUserAccountId;
			//eval(userAccountId).value = allUserAccountId;
		}
		if(userAccountName != null && userAccountName != ""){
			document.getElementById(userAccountName).value=allUserAccountName;
			//eval(userAccountName).value = allUserAccountName;
		}
		if(userLeaf != null && userLeaf != ""){
			document.getElementById(userLeaf).value=allUserLeaf;
			//eval(userLeaf).value = allUserLeaf;
		}
	}
...
/**取组织树对象**/
function getFreeTreeObj(){
	var _text = document.getElementById('_freeTreeDiv_text').value;
	var _id = document.getElementById('_freeTreeDiv_id').value;
	var _contractNumber = document.getElementById('_freeTreeDiv_contractNumber').value;
	var _email = document.getElementById('_freeTreeDiv_email').value;
	var _sex = document.getElementById('_freeTreeDiv_sex').value;
	var _parentId = document.getElementById('_freeTreeDiv_parentId').value;
	var _thisType = document.getElementById('_freeTreeDiv_thisType').value;
	var _accountId = document.getElementById('_freeTreeDiv_accountId').value;
	var _accountName = document.getElementById('_freeTreeDiv_accountName').value;
	var _leaf = document.getElementById('_freeTreeDiv_leaf').value;
	var obj = new Object();
	obj.id = _id;
	obj.text = _text;
	obj.contractNumber = _contractNumber;
	obj.email = _email;
	obj.sex = _sex;
	obj.parentId = _parentId;
	obj.thisType = _thisType;
	obj.accountId = _accountId;
