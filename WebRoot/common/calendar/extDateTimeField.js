...
Ext.ux.form.DateTimeField = Ext.extend(Ext.form.DateField, {
	constructor:function(config){
        ...
        this.triggerClass = 'x-form-date-trigger-height-26px x-form-border-0px';
        Ext.ux.form.DateTimeField.superclass.constructor.apply(this, arguments);
	},
    dateFormat: 'Y-m-d',
    timeFormat: 'H:i:s',
    defaultAutoCreate: {
    ...
    onTriggerClick: function(){
        ...
        this.menu.picker.setValue(dateValue || new Date());
        ...
        if(dateValue==""){
            //为了避免日历失去焦点时触发的校验事件，作如下操作：
            //如果日历无默认值，则初始值为空。此时，给它一默认值（当前日期）并隐藏默认值，效果和没有输入一样。
            //完成日期选择后，将样式还原。
            dateValue = new Date();
            dateValue = dateValue.format(calformat);
            inputobj.value = dateValue;
            inputobj.className+=" calenderInputHidden";
        }
        
        SelectDate(inputobj,calformat,this.startDate,this.endDate);
...
Ext.reg('ZTESOFT.timefield', Ext.ux.form.DateTimeField);
...
Ext.reg('ZTESOFT.datefield', Ext.ux.form.DateTimeField);
