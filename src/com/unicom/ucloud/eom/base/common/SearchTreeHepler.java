...
public class SearchTreeHepler {
     ...
     * @since
     */
    public static String buildXMLString(List<TreeSearchEntity> treeDatas) {
        ...
        for (TreeSearchEntity treeEntity : treeDatas) {
            ...
            if (!CommonUtils.isEmpty(treeEntity.getNoteType()) && treeEntity.getNoteType() == 1) {
                ...
                treeNode.addAttribute("leaf", "0");
                treeNode.addAttribute("hasChild", "1");
                treeNode.addAttribute("xmlSource", "asyn_add.jsp");
            } else {
            ...
            if(!CommonUtils.isEmpty(treeEntity.getIsSelect()) && treeEntity.getIsSelect() == 1){
                treeNode.addAttribute("isSelected", "1");
            }else{
            ...
            if(treeEntity.getAccountId() != null){
                ...
                treeNode.addAttribute("accountName", treeEntity.getAccountName());
            }
            treeNode.addAttribute("contractNumber", treeEntity.getContanctNumber());
            if(!CommonUtils.isEmpty(treeEntity.getDefaultOpen()) && treeEntity.getDefaultOpen() == 1){
                treeNode.addAttribute("defaultOpen", "1");
            }else{
                treeNode.addAttribute("defaultOpen", "0");
            }
            treeNode.addAttribute("email", treeEntity.getEmail());
            treeNode.addAttribute("sex", treeEntity.getSex());
        }
    ...
    private static void sort(List<TreeSearchEntity> assingTree) {

        // 3.最后按结点ID，小的排前面
        Collections.sort(assingTree, new Comparator<TreeSearchEntity>() {
            @Override
            public int compare(TreeSearchEntity node1, TreeSearchEntity node2) {
