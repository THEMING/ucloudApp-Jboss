package com.unicom.ucloud.eom.e19.service;

import java.util.Map;
import java.util.List;

import org.json.JSONObject;

import org.springframework.beans.factory.annotation.Autowired; 
import org.springframework.stereotype.Service; 

import com.unicom.ucloud.eom.base.service.BaseServiceImpl; 
import com.unicom.ucloud.eom.e19.dao.IEomCardSheetDAO; 

/** 
 * 代码说明 
 * @version 1.0
 * @date 2013-01-17
 * @author Jerry
 */ 
@Service
public class EomCardSheetServiceImpl extends BaseServiceImpl implements IEomCardSheetService {

    @Autowired
    private IEomCardSheetDAO eomCardSheetDAO;

    @Override
    public List<Map<String, Object>> qryListByParam(JSONObject jsonObj) throws Exception{ 
        return eomCardSheetDAO.qryListByParam(jsonObj);
    }

    @Override
    public void save(JSONObject jsonObj) throws Exception{
        eomCardSheetDAO.save(jsonObj);
    }

    @Override
    public void delete(JSONObject jsonObj) throws Exception{
        eomCardSheetDAO.delete(jsonObj);
    }

    @Override
    public void update(JSONObject jsonObj) throws Exception{
        eomCardSheetDAO.update(jsonObj);
    }

}
